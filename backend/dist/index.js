...
api.post("/api/users", async (req, res) => {
    ...
    // save to database
    await pool.query("INSERT INTO users (username, password, email) VALUES ($1, $2, $3)", [regDetails.username, hash, regDetails.email]);
...
api.post("/api/users/login", async (req, res) => {
    ...
    const hash = await pool.query(`SELECT password FROM users WHERE email = '${loginDetails.email}'`);
    if (hash.rowCount === 0) {
        return res.status(400).send("Invalid username or password");
    }
    // compare passwords and email
    const isMatch = await bcrypt.compare(loginDetails.password, hash.rows[0].password);
    if (!isMatch) {
...
api.post("/api/posts", async (req, res) => {
    ...
    await pool.query("INSERT INTO posts (title, created_at, content, user_id, like_count) VALUES ($1, $2, $3, $4, $5)", [reqInput.title, reqInput.createdAt, reqInput.content, reqInput.userId, 0]);
...
api.get("/api/posts/:id", async (req, res) => {
    ...
    const { rowCount, rows } = await pool.query(`SELECT * FROM post WHERE id = ${postId} ;`);
    if (rowCount === 0) {
        return res.status(404).send("Not found");
    }
...
api.delete("/api/posts/:id", async (req, res) => {
    ...
    await pool
        ...
        .catch((error) => res.status(500).send(new Error("Something went wrong." + error)))
...
api.post("/api/comments", async (req, res) => {
    ...
    await pool
        .query(`INSERT INTO comments (created_at, message, post_id,user_id) VALUES (${new Date().toISOString()}, ${comment.message}, ${comment.postId}, ${comment.userId});`)
